...
const getCoins = async(query) => {
    try {
        ...
        var activewallets = await fetch(`https://explorer.win.win/ext/getactivewalletscount`);
        activewallets = activewallets.active_wallets_count;
        return Promise.resolve(coins.map(c => ({...c, avgBlockTime, avgMNTime, activewallets })));
    } catch (err) {
...
// Request the coins for a month.
const getCoinsMonth = query => fetch(`${ api }/coin/month`, query);
...
// Request the transactions for a month.
const getTXsMonth = query => fetch(`${ api }/tx/month`, query);
